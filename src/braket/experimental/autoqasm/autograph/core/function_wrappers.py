...
from braket.experimental.autoqasm.autograph.operators import variables
...
class FunctionScope(object):
  ...
  def __init__(self, function_name, scope_name, options):
    ...
    if use_name_scope:
      raise NotImplementedError("name_scopes is TensorFlow-specific")

    use_auto_deps = self.options.uses(converter.Feature.AUTO_CONTROL_DEPS)
    self.use_auto_deps = use_auto_deps
    if use_auto_deps:
      raise NotImplementedError("auto_control_deps is TensorFlow-specific")
