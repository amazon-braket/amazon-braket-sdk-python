...
from aqx.qdk.circuits.circuit import Circuit
from aqx.qdk.tasks import QuantumTask


class AwsQuantumTask(QuantumTask):
    """AWS Qx implementation of a quantum task."""
    ...
    def from_circuit(
        ...
        """
        AwsQuantumTask factory method that serializes the circuit, submits to AWS Qx, and returns
        back a AwsQuantumTask tracking the execution.
    ...
    def cancel(self) -> None:
        """Cancel the quantum task. This cancels the future and the task in AWS Qx."""
        self._future.cancel()
        self._aws_session.cancel_quantum_task(self._arn)

    def metadata(self, use_cached_value: bool = False) -> Dict[str, Any]:
        """
        Get task metadata defined in AWS Qx.

        Args:
            use_cached_value (bool, optional): If true returns the last value retrieved from
                AWS Qx GetQuantumTask API else the API is called and the cache is updated.
                Default = False.

        Returns:
            Dict[str, Any]: The AWS Qx GetQuantumTask API response. TODO: INSERT BOTO3 DOC LINK.
            If `use_cached_value` is True then AWS Qx is not called and the last value retrieved
            is returned.
        """
    ...
    def state(self, use_cached_value: bool = False) -> str:
        ...
        Returns:
            ...
            in the AWS Qx GetQuantumTask API call. TODO: INSERT BOTO3 DOC LINK. If
            `use_cached_value` is True then AWS Qx is not called and the last value retrieved
            is returned.
    ...
    def result(self) -> AwsQuantumTaskResult:
        """
        Get the quantum task result by polling AWS Qx to see if the task is completed. Once the
        task is completed the result is retrieved from S3 and returned as a QuantumTaskResult.
