...
@pytest.fixture
def mock_qpu_gates():
    supported_gates = ['ccnot', 'cnot', 'cphaseshift', 'cswap', 'swap', 'iswap', 'pswap', 'ecr', 'cy', 'cz', 'xy', \
            'zz', 'h', 'i', 'phaseshift', 'rx', 'ry', 'v', 'vi', 'x', 'y', 'z']
...
@pytest.mark.parametrize(
    ...
    [
        Circuit(),
        Circuit().h(range(4)).cnot(0, 5).pswap(0, 1, np.pi/4).xy(0, 1, 0.5).cphaseshift(0, 1, np.pi/4),
        Circuit().swap(0, 1).rx(0, 0.5).v(1).h(2).add_verbatim_box(
            Circuit().cz(0, 1).cz(0, 6).prx(0, np.pi/4, np.pi/5)
        ).z(3), 
        Circuit().add_verbatim_box(
            Circuit().cz(0, 2).prx(0, 0.5, 0.5)
        ).add_verbatim_box(
            Circuit().cz(0, 4).cz(3, 6)
        ),
        Circuit().add_verbatim_box(
            Circuit()
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        Circuit().h(0).add_verbatim_box(
            Circuit().cz(1, 2).prx(range(5), np.pi/4, np.pi/2).cz(2, 6)
