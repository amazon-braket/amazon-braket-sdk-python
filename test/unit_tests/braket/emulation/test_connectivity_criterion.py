...
def test_complete_graph_instantation_with_num_qubits():
    ...
    assert nx.utils.graphs_equal(criterion._connectivity_graph, nx.complete_graph(num_qubits, create_using=nx.DiGraph()))
...
def test_complete_graph_instantation_with_qubit_labels():
    ...
    circuit = Circuit().add_verbatim_box(vb)
    criterion.validate(circuit)
    assert nx.utils.graphs_equal(criterion._connectivity_graph, nx.complete_graph(qubit_labels, create_using=nx.DiGraph()))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        Circuit().add_verbatim_box(
            Circuit().cnot(1, 10).cphaseshift01(2, 11, np.pi/4)
        ).x(4).add_verbatim_box(
            Circuit().swap(2, 10)
        )
    ]
)
...
@pytest.mark.parametrize(
    "circuit",
    [
        Circuit().x(4).add_verbatim_box(
            Circuit().x(4)
        ), 
        Circuit().x(110).add_verbatim_box(
            Circuit().phaseshift(4, np.pi/4)
        ), 
        Circuit().add_verbatim_box(
            Circuit().cnot(1, 10).cphaseshift01(2, 11, np.pi/4)
        ).x(4).add_verbatim_box(
