...
def test_standardize_ionq_device_properties_json(reduced_ionq_device_capabilities_json):
    device_properties_json = standardize_ionq_device_properties_json(reduced_ionq_device_capabilities_json)
    ...
    for i in range(25):
        ...
        if i!=3:
            ...
            assert device_em_properties.twoQubitProperties[edge].twoQubitGateFidelity[0].gateName=="MS"
            assert device_em_properties.twoQubitProperties[edge].twoQubitGateFidelity[0].fidelity==0.12345
            assert device_em_properties.twoQubitProperties[edge].twoQubitGateFidelity[0].standardError==None
            
    assert device_em_properties.supportedResultTypes == valid_supportedResultTypes
    assert device_em_properties.errorMitigation == {Debias: ErrorMitigationProperties(minimumShots=2500)}
