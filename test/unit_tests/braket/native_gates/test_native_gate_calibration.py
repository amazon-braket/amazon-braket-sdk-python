...
def pulse_sequence(frame):
    return PulseSequence().barrier(qubits_or_frames=[frame]).delay(qubits_or_frames=[frame], duration=1000)
...
def test_filter_data(pulse_sequence):
    ...
    assert expected_calibration == calibration.filter_data(calibration_key)
